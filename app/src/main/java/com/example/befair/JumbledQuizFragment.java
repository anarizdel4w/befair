                ...
                cd.setOnDismissListener(new DialogInterface.OnDismissListener() {
                    ...
                    public void onDismiss(DialogInterface dialog) {
                        if (ct != null) {
                            ...
                            ct = new CountDownTimer(remainingTime, 1000) {
                                @Override
                                public void onTick(long millisUntilFinished) {
                                    long secondsRemaining = millisUntilFinished / 1000;
                                    timeLeft = millisUntilFinished;
    ...
    private void increaseTimer(View view) {
        ...
        ct = new CountDownTimer(timeLeft, 1000) {
            ...
            public void onFinish() {
                ...
                if (isAnsweredOne && isAnsweredTwo && isAnsweredThree && isAnsweredFour && isAnsweredFive) {
                    // Navigate to the success fragment
                    Bundle args = new Bundle();
                    args.putString("LessonName", lessonName);
                    Navigation.findNavController(view).navigate(R.id.action_jumbledQuizFragment_to_quizPassFragment, args);
                } else {
                    // Navigate to the failure fragment
                    Bundle args = new Bundle();
                    args.putString("LessonName", lessonName);
