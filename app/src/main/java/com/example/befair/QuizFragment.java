...
//                    ct.cancel();
    ...
    private void setAnswer() {
        ...
        // Concatenate underscores based on the length of the correct answer
        for (int i = 0; i < answerCharacter.length; i++) {
            displayBuilder.append("_ ");
        }

        String display = displayBuilder.toString();
        binding.quizAnswer.setText(display);
    }
    ...
    private void incrementTimer(String character, View view) {
        String answer = binding.quizAnswer.getText().toString();
        answer = character;
        String displayAnswer = answer.replaceAll("_", "_ ");
        displayAnswer = displayAnswer.trim();
        binding.quizAnswer.setText(displayAnswer);
        /*ct.cancel();
        timeLeft += 2000;
        ct = new CountDownTimer(timeLeft, 1000) {
            @Override
